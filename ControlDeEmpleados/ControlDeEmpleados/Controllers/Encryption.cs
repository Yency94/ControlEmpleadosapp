...
namespace ControlDeEmpleados.Controllers
{
    public class Encryption
    {
        ...
        public String EncryptKeySecond(string cadena)
        {
            String _resultado = string.Empty;
            byte[] encriptar = System.Text.Encoding.Unicode.GetBytes(cadena);
            _resultado = Convert.ToBase64String(encriptar);
            return _resultado;
        }
        public string DecryptKeySecond(string clave)
        {
            String _resultado = string.Empty;
            byte[] desencriptar = Convert.FromBase64String(clave);
            _resultado = System.Text.Encoding.Unicode.GetString(desencriptar);
            return _resultado;
        }
    }
}